...
import com.mysampleapp.demo.UserData;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void syncUserSettings() {
        ...
        if (AWSMobileClient.defaultMobileClient().getIdentityManager().isUserSignedIn()) {
            Log.i(LOG_TAG, "priyanka starting to sync user_settings");
            ...
            userSettings.getDataset().synchronize(new DefaultSyncCallback() {
                ...
                public void onSuccess(final Dataset dataset, final List<Record> updatedRecords) {
                    ...
                    //final TextView syncStatusTxt = (TextView) findViewById(R.id.syncStatus);
                    //syncStatusTxt.setText("Successfully synced to Cognito");
                ...
                @Override
                public void onFailure(DataStorageException d) {
                    Log.d(LOG_TAG, "failed to sync user settings");
                    //final TextView syncStatusTxt = (TextView) findViewById(R.id.syncStatus);
                    //syncStatusTxt.setText("Failed to sync to Cognito");
                }
            });
        }
    }
    ...
    private void syncUserData() {
        // sync only if user is signed in
        if (AWSMobileClient.defaultMobileClient().getIdentityManager().isUserSignedIn()) {
            Log.i(LOG_TAG, "priyanka starting to sync user_data");
            final UserData userData = UserData.getInstance();
            userData.getDataset().synchronize(new DefaultSyncCallback() {
                private int syncStatus;

                @Override
                public void onSuccess(final Dataset dataset, final List<Record> updatedRecords) {
                    super.onSuccess(dataset, updatedRecords);
                    Log.d(LOG_TAG, "successfully synced user data");
                ...
                @Override
                public void onFailure(DataStorageException d) {
                    Log.d(LOG_TAG, "failed to sync user data");
                    //final TextView syncStatusTxt = (TextView) findViewById(R.id.syncStatus);
                    //syncStatusTxt.setText("Failed to sync to Cognito");
                }
            });
